...
def get_content(html):
    ...
    items = soup.find_all(
        "div",
        class_="iva-item-root-G3n7v photo-slider-slider-3tEix "
    ...
    for item in items:
        price = item.find(
            "span", class_="price-root-1n2wM price-listRedesign-2OaSA"
        )
        if price:
        ...
        cats.append(
            {
                "title": item.find(
                    "a",
                    class_="link-link-39EVK link-design-default-2sPEv "
                ...
                "price": price,
                "metro": item.find(
                    "div", class_="geo-root-1pUZ8 iva-item-geo-1Ocpg"
                )
                .get_text()
                .replace("\xa0", " "),
            }
...
def save_file(items, path):
    with open(path, "w", newline="", encoding="utf-8") as file:
        ...
        for item in items:
            writer.writerow(
                [item["title"], item["link"], item["price"], item["metro"]]
            )
